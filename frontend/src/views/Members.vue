		...
		<el-table :data="items" v-loading="loading" border stripe class="w-full">
			...
			<el-table-column prop="gender" label="性别" width="80" align="center">
				<template #default="{ row }">
					<el-tag :type="row.gender === 'MALE' ? 'info' : 'danger'" effect="plain" size="small">
...
function openCreate() {
	...
	resetForm()
	// Clear validation after opening
	...
	await formRef.value.validate(async (valid) => {
		if (valid) {
			...
			try {
				...
				if (editingId.value) {
					await http.put(`/members/${editingId.value}`, payload)
				} else {
